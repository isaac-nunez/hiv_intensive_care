...
data_uci_belaunz <- data_uci_bibi %>% 
  left_join(select(hospitalizacion, registro, fech_ing, fech_egr, si_urg, tot_veces_urg, num_estancia, 
                   entrada, tipo_ingreso, paciente_qx), 
            by = "registro") %>% 
  mutate(fecha_egr_hosp = ymd(fech_egr),
         fecha_ing_hosp = ymd(fech_ing),
         fecha_ing_uti = ymd(fecha_ing_uti),
         fecha_egr_uti = ymd(fecha_egr_uti),
         defuncion = ymd(defuncion)) %>% 
  filter(fech_ing-1 <= fecha_ing_uti & fech_egr+5 >= fecha_egr_uti) %>% 
...
uti_basic <- uti_bas %>% #Pulimos ciertas variables de la base basic + UTI, no se hizo en el paso previo por probable glitch (creo que no lo corría bien)
  ...
  rename(fecha_ing = fech_ing, fecha_egr = fech_egr) %>%
  select(registro, fecha_ing, fecha_egr, birth_d, edad_al_ingreso, everything()) %>%
  group_by(fecha_ing) %>%
  slice(1) 

uti_periodo <- uti_in %>% #Aquí filtramos la info de la base que me pasó Belaunzarán, según la fecha. CREO QUE ESTO NO ES NECESARIO, FIJARSE SI LA USO DESPUÉS 
  filter(fech_ing >= "2003-01-01")

uti_basic_prev <- uti_basic %>% #Aquí sacamos a los pacientes que hayan tenido una fecha de ingreso a la terapia previa a la fecha de enrolamiento
  filter(enrol_d < fecha_ing) #CREO QUE ESTO NO ES NECESARIO, FIJARSE SI LO UTILIZO DESPUÉS

uti_unique_patient <- uti_basic %>% #Aquí estoy filtrando según el registro, sólo para obtener variables sociodemográficas
  group_by(registro) %>%
  slice(1) #CREO QUE ESTO NO ES NECESARIO, FIJARSE SI LO UTILIZO DESPUÉS
```

```{r Depuración de información sobre carga viral y CD4, include = F}
...
#Cuando complete la base de microbiología, probablemente incluya solo las cargas que hayan tenido durante su hospitalización o las previas a ella en caso de no tener carga viral en la hospitalización (lo cual podría suceder en caso de que la previa sea muy reciente)
...
#Aquí simplifico la info de la base de cargas virales, filtrando según fecha de hospitalización
rna_mod <- rna %>% 
  ...
  mutate(rna_d_hosp = if_else(rna_d >= fecha_ing_hosp & rna_d <= fecha_egr_hosp, rna_d,
                                   if_else(fecha_ing_hosp - rna_d <= 365 , rna_d, ymd("1900-09-09"))),
         rna_v_hosp = if_else(rna_d >= fecha_ing_hosp & rna_d <= fecha_egr_hosp, rna_v,
                                   if_else(fecha_ing_hosp -rna_d <=365, rna_v, -123456))) %>% 
  group_by(registro) 

rna_mod$rna_d_hosp[rna_mod$rna_d_hosp == "1900-09-09"] <- NA
rna_mod$rna_v_hosp[rna_mod$rna_v_hosp == -123456] <- NA

rna_mod <- rna_mod %>% 
  arrange(desc(rna_d_hosp)) %>% 
  slice(1)

sum(is.na(rna_mod$rna_v_hosp))

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

uti_basic_rna <- uti_basic %>% 
  left_join(rna_mod, by = "registro")

rna_lvl_neardate <- uti_basic_rna %>% #Aquí especifico que tome las cargas virales sólo durante la hospitalización (y en ausencia de, entonces previos a ella)
  mutate(diff_rna = fecha_ing-rna_d) %>%
  filter(diff_rna < 90 & diff_rna >-30)


%>%
  group_by(fecha_ing)%>%
  summarise(detectable = if_else(all(rna_v < 200), "indetectable", "detectable"))

uti_basic_2 <- uti_basic %>%
  left_join(rna_lvl_neardate, by = "fecha_ing") 

#Aquí analizamos los CD4 POR FECHA, es decir, por ingreso
#Entran aquellas que se hayan tomado en los 90 días previos o en el mes posterior a al ingreso
#Probablemente cuando termine la base de microbiología incluya primordialmente los conteos tomados durante la hospitalización
...
cd4_count_neardate <- y1 %>%
  mutate(diff_cd4 = fecha_ing-cd4_d) %>%
  filter(diff_cd4 < 90 | diff_cd4 >-30) %>%
  summarise(cuenta_cd4 = if_else(any(cd4_v < 200), "below 200", "above 200"))

uti_basic_3 <- uti_basic %>%
  left_join(cd4_count_neardate, by = "fecha_ing")
```

```{r Depurando la información sobre tratamiento antirretroviral, include = F}
art_1 <- art %>%
  filter(registro %in% uti_basic$registro) %>%
  group_by(registro, inicioesquema, finalesquema) %>%
  count(cveantirretroviral) %>%
  mutate(esquema = if_else(is.na(cveantirretroviral), "No treatment reg",
                           paste(cveantirretroviral, collapse = ",")),
         es_haart = if_else(str_count(esquema, ",") > 1 | esquema == "Atripla", 
                            "haart", "no haart")) %>%
  select(registro, inicioesquema, finalesquema, esquema, es_haart) %>%
  distinct()

#Aquí estoy definiendo tx_prev_ing como que haya iniciado el esquema al menos 3 meses previo a su ingreso a la UTI y que no haya finalizado su esquema previo al ingreso
#OJO con esta variable, probablemente haya que redefinirla simplemente como cualquier tratamiento previo al ingreso HOSPITALARIO (no solo a UTI)
z <- uti_basic %>%
  right_join(art_1, by = "registro") %>%
  mutate(tx_prev_ing = if_else(!is.na(neardate(registro, registro, 
                                        fecha_ing, inicioesquema, "prior")) &
                                 (finalesquema > fecha_ing | is.na(finalesquema) &
                                    (fecha_ing - inicioesquema) >=90), T,
                                F)) %>%
  select(registro, fecha_ing, fecha_egr, inicioesquema, finalesquema, 
         esquema, es_haart, tx_prev_ing, everything()) 

z1 <- z %>%
  filter(tx_prev_ing == T) %>% 
  group_by(fecha_ing) %>%
  slice(1)#Solo pacientes cuyo tx inició previo al ingreso a UTI
```

```{r Uniendo la información generada en chunks previos en un solo dataframe, include = F}
#Construyendo tabla de variables demográficas
uti_basic_22 <- uti_basic_2 %>%
  select(registro, fecha_ing, detectable)

uti_basic_33 <- uti_basic_3 %>%
  select(registro, fecha_ing, cuenta_cd4)

uti_z1 <- z1 %>%
  select(registro, fecha_ing, inicioesquema, finalesquema, esquema, es_haart, tx_prev_ing)

#Uniendo los dataframes
uti_basico <- uti_basic %>%
  left_join(uti_basic_22, by = "fecha_ing") %>%
  left_join(uti_basic_33, by = "fecha_ing") %>%
  left_join(uti_z1, by = "fecha_ing") %>%
  rename(registro_ = registro.x) %>%
  select(registro_:enrol_d, gender, duracion_estancia, detectable, cuenta_cd4, 
         inicioesquema, finalesquema, esquema, es_haart, tx_prev_ing, ing_cie10_dx,ing_dx_prin, egre_cie10_dx, egr_dx) %>%
  mutate(tx_prev_ing = if_else(is.na(tx_prev_ing),F, T),
         ing_dx = tolower(ing_dx_prin) , 
         egr_dx = tolower(ing_dx_prin)) %>%
  rename(registro = registro_)

uti_basico_1 <- uti_basico %>%
  mutate(motivo_ingreso = 
  ...
  mutate(fecha_ing_ = fecha_ing) %>%
  separate(fecha_ing_, 4, into = c("year_ing", "mes_d_ing")) %>%
  mutate(year_ing = as.integer(year_ing),
         art_prev = ifelse(tx_prev_ing == T, "ART previo al ingreso",
                            "Sin ART previo al ingreso"))
```

```{r Generando el tiempo de supervivencia, include = F}
#Ojo, aquí no estoy censurando a un año, solo estoy calculando el tiempo de supervivencia en base a si tiene fecha de muerte o pérdida de seguimiento descrita por la base (la última visita es el referente para el tiempo de supervivencia en quienes no hayan tenifdo algún evento)
uti_basico_2 <- uti_basico_1 %>%
  left_join(follow, by = "registro") %>%
  left_join(visit, by = "registro") %>%
  group_by(registro) %>%
  arrange(desc(visit_d), .by_group = T) %>%
  mutate(last_visit = first(visit_d)) %>%
  select(everything(), -visit_d) %>%
  distinct() %>%
  mutate(muerte = if_else(!is.na(death_y), "Si", "No"),
         death_d = ymd(death_d),
         drop_d = ymd(drop_d),
         surv_time = if_else(!is.na(death_d),death_d-fecha_ing,
                             if_else(!is.na(drop_d) & drop_d > fecha_egr, drop_d-fecha_ing, 
                                     if_else(!is.na(last_visit) & last_visit > fecha_egr, 
                                             last_visit-fecha_ing, fecha_egr-fecha_ing))),
         surv_time = if_else(surv_time == 0, surv_time + 1, surv_time),
        evento = if_else(muerte == "Si", 1, 0),
        asoc_vih = ifelse(any(str_detect(ing_dx, "vih")|
                             str_detect(ing_dx, "tuberculosis")|
                           str_detect(ing_dx, "cripto")|
                              str_detect(ing_dx, "citom")|
                             str_detect(ing_dx, "mico")|
                              str_detect(ing_dx, "cocci")), "si", "no")) %>%
  distinct()
```

```{r Agregando cargas y cd4 cuantitativas más cercanas al internamiento, include = F}
#Base con la carga del primer internamiento
carga_rna <- uti_basico_2 %>%
  right_join(rna, by = "registro") %>%
  filter(!is.na(detectable))%>%
  group_by (registro) %>%
  mutate(rna_d = ymd(rna_d),
         diff_rna = ifelse(fecha_ing-rna_d<0, 
                          -(fecha_ing-rna_d), fecha_ing-rna_d )) %>%
  filter(diff_rna == min(diff_rna)) %>%
  select(registro, fecha_ing, rna_d, diff_rna, rna_v)
  
#Base con el conteo del primer internamiento
conteo_cd4 <- uti_basico_2 %>%
  right_join(cd4, by = "registro") %>%
  filter(!is.na(cuenta_cd4))%>%
  group_by (registro) %>%
  mutate(cd4_d = dmy(cd4_d),
         diff_cd4 = ifelse(fecha_ing-cd4_d<0, 
                          -(fecha_ing-cd4_d), fecha_ing-cd4_d ))%>%
  filter(diff_cd4 == min(diff_cd4)) %>%
  select(registro, fecha_ing, cd4_d, diff_cd4, cd4_v)

uti_basico_4.5 <- uti_basico_2 %>%
  left_join(carga_rna, by = "registro") %>%
  left_join(conteo_cd4, by = "registro") 
```

```{r Censurando los tiempos de supervivencia según el tiempo de interés y los eventos, include = F}
#Base con solo el primer internamiento a modo de supervivencia
uti_basico_5 <- uti_basico_4.5 %>%
  group_by(registro) %>%
  slice(1) %>%
  mutate(surv_time_m = if_else(surv_time == 0, surv_time+1, surv_time),
         muerte_uti = if_else(!is.na(death_d) & death_d <= fecha_egr, 1, 0),
         muerte_28 = if_else(evento == 1 & surv_time_m <= 28, 1, 0),
         muerte_60 = if_else(evento == 1 & surv_time_m <= 60, 1, 0),
         muerte_1a = if_else(evento == 1 & surv_time_m <= 365, 1, 0),
         surv_time_cens1a = ifelse(surv_time_m > 365, 365, surv_time_m),
         surv_time_cens28 = ifelse(surv_time_m > 28, 28, surv_time_m),
         surv_time_cens60 = ifelse(surv_time_m > 60, 60, surv_time_m),
         perdido_seg = if_else(evento == 0 & surv_time < 180, T, F),
         fecha_ing = fecha_ing.x) %>% 
  filter(registro != 259105 &
           registro != 219863)#Estos pacientes adquirieron vih posterior al ingreso o no tienen VIH
...
mediana <- uti_km %>%
  filter(tx_prev_ing == T) %>%
  group_by(registro) %>%
  summarise(seguimiento = fecha_ing-inicioesquema) %>%
  arrange(seguimiento)
```

```{r Modelos de Cox (probablemente sean inútiles por los que hice después, confirmar si puedo borrarlos), include = F}
cox_diferentes <- function(x){
  summary(coxph(Surv(x, evento) ~ gender + edad_al_ingreso + detectable + cuenta_cd4 + tx_prev_ing +
                   motivo_ingreso, data =  uti_basico_5))
}

res.cox <- coxph(Surv(surv_time_m, evento) ~ gender + edad_al_ingreso + detectable + cuenta_cd4 + art_prev +
                   motivo_ingreso, data =  uti_basico_5)

```

```{r Unión de base final con micro, include = F}
#"Naive a tratamiento?"
basic_naive <- basic %>%
  select(registro, naive_y)

uti_km <- uti_km %>%
  left_join(basic_naive, by = "registro")

art_fil <- art %>%
  filter(registro %in% uti_km$registro[uti_km$tx_prev_ing == F]) %>%
  select(registro, inicioesquema)

uti_km_nofinal <- uti_km %>%
  left_join(art_fil, by = "registro") %>%
  rename(inicioesquema = inicioesquema.y) %>%
  filter(!is.na(inicioesquema)) %>%
  mutate(date_dif = fecha_ing.x-inicioesquema) %>%
  filter(date_dif > 0)

uti_km <- uti_km %>%
  mutate(naive_al_ing = if_else(tx_prev_ing == T, "No", 
                         if_else(registro %in% uti_km_nofinal$registro, "No",
                                 if_else(!is.na(naive_y) & naive_y == "Sí", "Si", "Desc"))), 
         naive_logical =ifelse(naive_al_ing == "No", F, 
                               ifelse(naive_al_ing == "Si", T, NA)))
...
micro_3 <- micro_2 %>% 
  ...
  mutate(has_tuberculosis = if_else(any(has_tb == T), T,F),
         ...
         carga_intra_hosp = ifelse(any(tipo_prueba == "carga_vih"), as.double(microorg[tipo_prueba == "carga_vih"]), -12345),
         cd4_intra_hosp = ifelse(any(tipo_prueba == "cd4"), as.double(microorg[tipo_prueba == "cd4"]), -123)) %>% #Completar las cargas y los CD4 intrahospitalarios
  ungroup() %>% 
  filter(tipo_prueba != "carga_vih" & tipo_prueba != "cd4") %>% 
  group_by(registro) %>% 
  slice(1)

#Colocando NAs que no pudieron modificarse con dplyr
